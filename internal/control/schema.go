...
// BuyerUsersSQL creates the canonical buyer account table in PostgreSQL.
const BuyerUsersSQL = `
...
CREATE UNIQUE INDEX IF NOT EXISTS buyer_users_email_lower ON buyer_users (lower(email));
`

// SellerSchemaSQL creates seller account and model tables.
const SellerSchemaSQL = `
...
CREATE UNIQUE INDEX IF NOT EXISTS seller_users_email_lower ON seller_users (lower(email));
...
CREATE INDEX IF NOT EXISTS idx_seller_models_seller_id ON seller_models (seller_id);
`

// BillingSchemaSQL stores per-party wallets, token quotas/usage, transaction ledger, and inference match records.
const BillingSchemaSQL = `
CREATE TABLE IF NOT EXISTS buyer_billing (
...
CREATE INDEX IF NOT EXISTS idx_billing_tx_party_created ON billing_transactions (party_kind, party_id, created_at DESC);

CREATE TABLE IF NOT EXISTS inference_matches (
...
CREATE INDEX IF NOT EXISTS idx_inference_matches_buyer ON inference_matches (buyer_id, created_at DESC);
`

// BillingBackfillSQL inserts billing rows for existing accounts (idempotent).
const BillingBackfillSQL = `
